...
class ReviewsServices {
  ...
  static Future<List<ReviewsModel>> getReviewsByArtistId({required String artistId,required String accessToken}) async {
    final apiUrl = "${UrlConstants.getArtistReview}/$artistId";
    ...
    try {
      ...
      if (response.statusCode == 200) {
       ...
       print(stacktrace.toString());
       print("Error Artist Reviews: ${e.toString()}");
       return [];
    }
  }
  ...
  static Future<ReviewData> addReviewToArtist({required String artistId,required int rating,required String discription,required String accessToken}) async {
    const apiUrl = UrlConstants.addReview;
